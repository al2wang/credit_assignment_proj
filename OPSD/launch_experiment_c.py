...
if __name__ == '__main__':
    ...
    # Gamma values for the horizon ablation
    # 1.0 = Standard CPPO (Infinite horizon memory)
    # 0.999 = Optimal (Forgets early exploration gradually)
    # 0.99 = Aggressive (Too short, risk of off-policy drift)
    gammas = [1.0, 0.999, 0.99]
    
    alpha = 1.0
    budget = 2.5 

    base_dir = "/network/scratch/g/guangyuan.wang/credit_assignment/OPSD"  
    job_script_directory = os.path.join(base_dir, "temp_job_scripts_exp_c")
    out_dir = os.path.join(base_dir, "grpo_cppo", "logs_exp_c")
    ...
    for i, gamma in enumerate(gammas):
        ...
        run_name = f"exp_c_qwen4b_g{gamma}".replace(".", "p")
        ...
        bash_run_command = (
            ...
            f"--max_completion_length 4096 " # Expanded max length to observe recovery and length expansion
            f"--num_generations 8 "
            f"--temperature 1.0 " 
            f"--use_vllm "
            f"--use_peft "
            f"--vllm_mode colocate "
            f"--vllm_max_model_len 8192 " # Expanded context window to support longer proofs
            f"--logging_steps 5 " 
            f"--save_steps 50 "
            f"--beta 0.0 "
            f"--loss_type grpo "
            f"--scale_rewards group "
            f"--wandb_project GRPO_CPPO_Discounting_Ablation "
            f"--cppo_alpha {alpha} "
            f"--cppo_budget {budget} "
            f"--cppo_gamma {gamma} " # NEW ARGUMENT
            f"--report_to wandb"
        )
...
'''
    ...
    print(f"Total Experiment C discounting sweeps submitted to SLURM: {submitted_jobs}")