...
if __name__ == '__main__':
    ...
    exploration_modes = ["standard", "langevin_noise", "prm_guided"]
    
    # Fixed CPPO hyper-parameters for a controlled comparison
    alpha = 1.0
    budget = 2.5 

    base_dir = "/network/scratch/g/guangyuan.wang/credit_assignment/OPSD"  
    job_script_directory = os.path.join(base_dir, "temp_job_scripts_exp_b")
    out_dir = os.path.join(base_dir, "grpo_cppo", "logs_exp_b")
    ...
    for i, mode in enumerate(exploration_modes):
        
        run_name = f"exp_b_qwen4b_{mode}".replace(".", "p")
        ...
        bash_run_command = (
            ...
            f"--wandb_project GRPO_CPPO_Exploration_Ablation "
            f"--cppo_alpha {alpha} "
            f"--cppo_budget {budget} "
            f"--exploration_mode {mode} " # NEW ARGUMENT
            f"--report_to wandb"
        )
...
'''
    ...
    print(f"Total Experiment B exploration sweeps submitted to SLURM: {submitted_jobs}")