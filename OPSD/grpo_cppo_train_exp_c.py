...
class CPPOConfig(GRPOConfig):
    ...
    cppo_gamma: float = field(
        default=1.0,
        metadata={"help": "Discount factor for the cumulative prefix KL budget."}
    )
...
class CPPOGRPOTrainer(GRPOTrainer):
    def __init__(self, *args, **kwargs):
        ...
        self._cppo_metrics = {
            ...
            "response_length_k": [], 
            "rel_log_prob_error": [],
            "max_discounted_kl": [] # Track peak KL accumulation
        }
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # 1. Calculate PRM/MCTS Guided Dynamic Epsilon (Using UCB Oracle from Exp A/B)
        ...
        # =====================================================================
        # EXPERIMENT C: Discounted Prefix KL Budget Calculation
        # =====================================================================
        if self.args.cppo_gamma == 1.0:
            # Standard cumulative sum (inf memory)
            discounted_kl = torch.cumsum(per_token_kl, dim=1)
        else:
            # Vectorized O(L^2) calculation for the discounted sum: 
            # K_t = \sum_{i=1}^t gamma^{t-i} D_KL^{(i)}
            idx = torch.arange(seq_len, device=per_token_kl.device, dtype=torch.float32)
            # Create difference matrix where diff[i, j] = i - j
            diff_matrix = idx.unsqueeze(1) - idx.unsqueeze(0)
            # Mask out future positions (j > i)
            valid_mask = (diff_matrix >= 0).float()
            # Calculate gamma^(i-j) for valid entries
            gamma_matrix = (self.args.cppo_gamma ** diff_matrix) * valid_mask
            
            # Matrix multiply to compute sums across the sequence
            # (Batch, Seq) x (Seq, Seq).T -> (Batch, Seq)
            discounted_kl = torch.matmul(per_token_kl, gamma_matrix.T)
        # =====================================================================

        budget_mask = (discounted_kl <= self.args.cppo_budget).float()
        ...
        with torch.no_grad():
            ...
            self._cppo_metrics["max_discounted_kl"].append(discounted_kl.max().item())
...
if __name__ == "__main__":
    ...
    if script_args.run_config:
        ...
        full_wandb_run_name = (
            f"CPPO_gamma{training_args.cppo_gamma}_{model_name}_"
            f"lr{lr_str}_bs{effective_batch_size}"
        )

    print(f"\n{'='*80}")
    print(f"EXPERIMENT C: DISCOUNTING ABLATION")
    print(f"{'='*80}")
    print(f"WandB Run Name: {full_wandb_run_name}")
    print(f"Output Directory: {training_args.output_dir}")
    print(f"CPPO Gamma (Discounting): {training_args.cppo_gamma}")
    print(f"{'='*80}\n")
    ...
    if os.environ.get("LOCAL_RANK", "0") == "0":
        wandb.init(
            ...
            config={
                ...
                "cppo_gamma": training_args.cppo_gamma,
            },
        )
