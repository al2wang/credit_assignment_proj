...
import torch
import json
from context_truncation_utils import create_truncated_causal_mask
...
class CPPOConfig(GRPOConfig):
    ...
    truncation_mode: str = field(
        default="full",
        metadata={"help": "Mode: 'full', 'markovian', 'sliding', 'sinks'"}
    )
    truncation_k: int = field(
        default=400,
        metadata={"help": "The context window horizon length."}
    )
...
class CPPOGRPOTrainer(GRPOTrainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # Calculate exact prompt lengths per batch item for the custom mask
        prompt_lens = prompt_mask.sum(dim=1).long()
        
        # =====================================================================
        # EXPERIMENT: Generate Truncated 4D Attention Mask
        # Note: If a 4D mask is provided, the SDPA backend natively handles it.
        # =====================================================================
        custom_4d_mask = create_truncated_causal_mask(
            input_ids=input_ids,
            prompt_lens=prompt_lens,
            mode=self.args.truncation_mode,
            k=self.args.truncation_k
        )
        
        # If custom_4d_mask is None, we fall back to the standard 2D padding mask.
        forward_mask = custom_4d_mask if custom_4d_mask is not None else attention_mask

        outputs = model(input_ids, attention_mask=forward_mask)
        logits = outputs.logits[:, :-1, :]
        
        with torch.inference_mode():
            if self.ref_model is None:
                with self.accelerator.unwrap_model(model).disable_adapter():
                    ref_outputs = model(input_ids, attention_mask=forward_mask)
            else:
                ref_outputs = self.ref_model(input_ids, attention_mask=forward_mask)
        ...
        # Calculate true IS ratio and reshape to isolate the group variance
        ...
        with torch.no_grad():
            ...
            # Periodically write dynamics to disk
            if self.state.global_step > 0 and self.state.global_step % self.args.logging_steps == 0:
                if self.accelerator.is_main_process:
                    log_dir = os.path.join(self.args.output_dir, "context_dynamics_logs")
                    os.makedirs(log_dir, exist_ok=True)
                    step_data = {
                        "step": self.state.global_step,
                        "mode": self.args.truncation_mode,
                        "var_rho_t": var_rho_t[0].cpu().numpy().tolist(),
                        "cum_kl": cum_kl[0].cpu().numpy().tolist(),
                        "step_kl": per_token_kl[0].cpu().numpy().tolist()
                    }
                    log_file = os.path.join(log_dir, f"{self.args.truncation_mode}_step_{self.state.global_step}.json")
...
if __name__ == "__main__":
    ...
    if script_args.run_config:
        ...
        full_wandb_run_name = (
            f"CPPO_{training_args.truncation_mode}_k{training_args.truncation_k}_{model_name}_"
            f"lr{lr_str}_bs{effective_batch_size}"
        )

    print(f"\n{'='*80}")
    print(f"EXPERIMENT: CONTEXT TRUNCATION DYNAMICS")
    print(f"{'='*80}")
    print(f"WandB Run Name: {full_wandb_run_name}")
    print(f"Output Directory: {training_args.output_dir}")
    print(f"Truncation Mode: {training_args.truncation_mode}")
    print(f"Context Horizon (k): {training_args.truncation_k}")
    print(f"{'='*80}\n")
    ...
    if os.environ.get("LOCAL_RANK", "0") == "0":
        wandb.init(
            ...
            config={
                ...
                "truncation_k": training_args.truncation_k,
            },
        )
