...
import torch
import json
from epistemic_prm_oracles import OptimisticUCTOracle, EpinetOracle
...
class CPPOConfig(GRPOConfig):
    ...
    uncertainty_mode: str = field(
        default="time_heuristic",
        metadata={"help": "Mode for dynamic bounds: 'time_heuristic', 'uct_guided', 'epinet_guided'"}
    )
...
class CPPOGRPOTrainer(GRPOTrainer):
    def __init__(self, *args, **kwargs):
        ...
        self.uct_oracle = None
        self.epinet_oracle = None
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # =====================================================================
        # EXPERIMENT D: Formalizing PRM Epistemic Uncertainty
        # =====================================================================
        base_eps = torch.full_like(ratio, self.args.cliprange)
        u_t_logged = 0.0

        if self.args.uncertainty_mode == "uct_guided":
            if self.uct_oracle is None:
                self.uct_oracle = OptimisticUCTOracle().to(input_ids.device)
            u_t = self.uct_oracle.simulate_or_fetch_uct(input_ids)[:, prompt_len - 1:]
            epsilon_t = base_eps * (1.0 + self.args.cppo_alpha * u_t)
            u_t_logged = u_t.mean().item()

        elif self.args.uncertainty_mode == "epinet_guided":
            if self.epinet_oracle is None:
                self.epinet_oracle = EpinetOracle().to(input_ids.device)
            u_t = self.epinet_oracle.simulate_or_fetch_epinet(input_ids)[:, prompt_len - 1:]
            epsilon_t = base_eps * (1.0 + self.args.cppo_alpha * u_t)
            u_t_logged = u_t.mean().item()

        else: # "time_heuristic"
            time_heuristic = positions / seq_len
            epsilon_t = self.args.cliprange * (1.0 + self.args.cppo_alpha * time_heuristic)
            epsilon_t = epsilon_t.unsqueeze(0).expand_as(ratio) 
            u_t_logged = time_heuristic.mean().item()
        # =====================================================================
        ...
        with torch.no_grad():
            ...
            self._cppo_metrics["mean_uncertainty_metric"].append(u_t_logged)
...
if __name__ == "__main__":
    ...
    if script_args.run_config:
        ...
        full_wandb_run_name = (
            f"CPPO_{training_args.uncertainty_mode}_{model_name}_"
            f"lr{lr_str}_bs{effective_batch_size}"
        )

    print(f"\n{'='*80}")
    print(f"EXPERIMENT D: EPISTEMIC UNCERTAINTY ABLATION")
    print(f"{'='*80}")
    print(f"WandB Run Name: {full_wandb_run_name}")
    print(f"Output Directory: {training_args.output_dir}")
    print(f"Uncertainty Mode: {training_args.uncertainty_mode}")
    print(f"{'='*80}\n")
    ...
    if os.environ.get("LOCAL_RANK", "0") == "0":
        wandb.init(
            ...
            config={
                ...
                "uncertainty_mode": training_args.uncertainty_mode,
            },
        )
