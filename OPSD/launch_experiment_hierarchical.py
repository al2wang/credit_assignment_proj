...
if __name__ == '__main__':
    ...
    # The four modes testing the impact of cross-sequence sharing and hierarchical RL
    advantage_modes = [
        "standard_grpo",         # Baseline: Standard batch mean advantage
        "knn_prompt_only",       # Replay Buffer: Only uses b(x_i) from historical KNN
        "hierarchical_static",   # Full Hierarchy: Fixed beta = 0.5
        "hierarchical_dynamic"   # Full Hierarchy: Uncertainty-guided beta schedule (Ours)
    ]
    ...
    job_script_directory = os.path.join(base_dir, "temp_job_scripts_hierarchical")
    out_dir = os.path.join(base_dir, "grpo_cppo", "logs_hierarchical")
    ...
    for i, mode in enumerate(advantage_modes):
        
        run_name = f"exp_hierarchical_qwen4b_{mode}".replace(".", "p")
        ...
        bash_run_command = (
            ...
            f"--wandb_project GRPO_CPPO_Hierarchical_Advantage "
            f"--cppo_alpha {alpha} "
            f"--cppo_budget {budget} "
            f"--advantage_mode {mode} " # NEW ARGUMENT
            f"--report_to wandb"
        )
...
'''
    ...
    print(f"Total Hierarchical Advantage sweeps submitted to SLURM: {submitted_jobs}")