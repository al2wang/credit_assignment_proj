...
if __name__ == '__main__':
    ...
    # Alpha=0.0 with a massive budget effectively disables CPPO to establish the GRPO baseline
    alphas = [0.0, 1.0]
    budgets = [9999.0, 2.5] 

    base_dir = "/network/scratch/g/guangyuan.wang/credit_assignment/OPSD"  
    job_script_directory = os.path.join(base_dir, "temp_job_scripts_exp_a")
    out_dir = os.path.join(base_dir, "grpo_cppo", "logs_exp_a")
    ...
    for i, (alpha, budget) in enumerate(zip(alphas, budgets)):
        
        run_name = f"exp_a_qwen4b_a{alpha}_b{budget}".replace(".", "p")
        ...
        bash_run_command = (
            ...
            f"--wandb_project GRPO_CPPO_IS_Dynamics "
            f"--cppo_alpha {alpha} "
...
'''
    ...
    print(f"Total Experiment A diagnostic sweeps submitted to SLURM: {submitted_jobs}")